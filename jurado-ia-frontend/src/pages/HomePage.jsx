...
function ResultsDisplay({ result }) {
    ...
    return (
      <div className="results-card">
        ...
        <div className="score"><strong>Pontuação:</strong> {result.pontuacao} / {result.pontuacao_maxima || 100}</div>
...
function SingleAnalysis() {
    ...
    const handleSubmit = async (e) => {
      ...
      const backendUrl = `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'}${endpoint}`;
      try {
    ...
    return (
      <div className="tab-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="criteria-input"><label htmlFor="criteria-single">Critérios de Avaliação</label><input id="criteria-single" type="text" value={criteria} onChange={(e) => setCriteria(e.target.value)} placeholder="Análise geral de qualidade" /></div>
...
function CompetitionAnalysis() {
    ...
    const handleSubmit = async (e) => {
      ...
      const backendUrl = `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'}/analyze/multiple`;
      try {
        const response = await axios.post(backendUrl, formData);
