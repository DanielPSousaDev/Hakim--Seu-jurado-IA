    ...
    def analyze(self, text: str, criteria: str = "Avaliação geral de qualidade") -> Dict[str, Any]:
        try:
            ...
            try:
                ...
                analysis = json.loads(content[start_idx:end_idx]) if start_idx != -1 and end_idx != 0 else {"pontuacao": 0, "feedback": content}
        ...
        try:
            ...
            if extension.lower() == '.pdf':
                with open(file_path, 'rb') as f:
                    ...
                    if reader.is_encrypted: return {"erro": "O arquivo PDF está criptografado."}
                    extracted_text = "".join(page.extract_text() or "" for page in reader.pages)
            else:
                return {"erro": f"Tipo de documento '{extension}' não suportado."}
            if not extracted_text.strip(): return {"erro": "Nenhum texto extraído do documento."}
